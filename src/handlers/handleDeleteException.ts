...
import { removeException } from "../services/removeException";
import { embedCons } from "../bot/embeds/_util";

export async function handleDeleteException(interaction: ChatInputCommandInteraction) {
  ...
  try {
    ...
    const exceptionMember = await removeException(clan.id, memberName);
    ...
    await interaction.editReply({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(`Removed **${exceptionMember.name}** from the exception list.`)
          .setColor(embedCons.color.INFO),
      ],
    });
  } catch (error) {
